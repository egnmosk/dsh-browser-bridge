// dsh-browser-bridge вЂ” model-facing browser_* tools over the extension bridge.
//
...
export function registerTools(ctx, bridge, config = {}) {
				...
				render: (_args, value) => [
					{
						...
						text:
							value.tabs.length === 0
								? "No tabs found."
								: value.tabs
										.map((t) => `[${t.id}]${t.active ? " (active)" : ""} ${t.title || "(no title)"} вЂ” ${t.url || ""}`)
										.join("\n"),
					},
				],
			},
	...
	ctx.tools.register(
		defineTool({
			...
			output: {
				...
				render: (_args, value) => [{ type: "text", text: `Activated tab [${value.tab.id}]: ${value.tab.title || "(no title)"} вЂ” ${value.tab.url || ""}` }],
			},
			...
			execute: async (args, exec) => {
				...
				return { ok: true, tab: value.tab ?? { id: args.tabId } };
			},
		})
	);
	...
	ctx.tools.register(
		defineTool({
				...
				render: (_args, value) => [
					{
						...
						text: value.error
							...
							: `Navigation (${value.action}) ok. Tab [${value.tab.id}]: ${value.tab.title || "(no title)"} вЂ” ${value.tab.url || ""}`,
					},
				],
			},
			...
			execute: async (args, exec) => {
				...
				return { ok: true, action, tab: value.tab ?? {}, error: value.error ?? undefined };
			},
		})
	);
				...
				render: (_args, value) => [
					{
						type: "text",
						text:
							`Page: ${value.title || "(no title)"} вЂ” ${value.url}\n` +
							(value.elements.length === 0
								? "No interactive elements found."
								: value.elements
										.map((e, i) => `${i + 1}. <${e.tag}>${e.role ? ` role=${e.role}` : ""}${e.type ? ` type=${e.type}` : ""} ${e.text ? JSON.stringify(truncate(e.text, 60)) : ""} в†’ ${e.selector}`)
										.join("\n")),
					},
				],
			},
	...
	ctx.tools.register(
		defineTool({
			...
			output: {
				...
				render: (_args, value) => [
					{ type: "text", text: `Page: ${value.title || "(no title)"} вЂ” ${value.url}\n\n${value.text}${value.truncated ? "\n\n(truncated)" : ""}` },
				],
			},
	...
	ctx.tools.register(
		defineTool({
			...
			execute: async (args, exec) => {
				...
				if (value.ok === false && value.error) return { ok: false, selector: args.selector, error: value.error };
				return { ok: true, selector: args.selector, tag: value.tag, text: value.text };
			},
		})
	);
	...
	ctx.tools.register(
		defineTool({
			...
			execute: async (args, exec) => {
				...
				if (value.ok === false && value.error) return { ok: false, key: args.key, error: value.error };
	...
	ctx.tools.register(
		defineTool({
			...
			execute: async (args, exec) => {
				...
				return { ok: true, found: value.found ?? false, selector: args.selector, timeoutMs: value.timeoutMs ?? args.timeoutMs ?? 10000, error: value.error ?? undefined };
			},
		})
	);
	...
	ctx.tools.register(
		defineTool({
			...
			description: `Capture a screenshot of the current tab (or a tabId). The image is saved to disk and the file path is returned вЂ” use your file tools to inspect it. ${NOT_CONNECTED_HINT}`,
			parameters: {
				tabId: { ...optInt, description: "Target tab; defaults to the active tab." },
			...
			execute: async (args, exec) => {
				...
				if (value.ok === false && value.error) return { ok: false, path: "", error: value.error };
				const file = bridge.saveScreenshot(value.dataUrl);
				return { ok: true, path: file, width: value.width ?? 0, height: value.height ?? 0, bytes: value.bytes ?? 0 };
			},
		})
	);
	...
	ctx.tools.register(
		defineTool({
			...
			execute: async (args, exec) => {
				...
				if (value.ok === false && value.error) return { ok: false, result: null, serialized: false, error: value.error };
				return { ok: true, result: value.result ?? null, serialized: value.serialized ?? false };
			},
		})
	);
}
